...
internal class DLangParser(private val builder: PsiBuilder) {
        ...
        if (i === DlangTypes.ID) {
            if (peekIs(DlangTypes.OP_NOT)) {
                if (!parseTemplateInstance()) {
                    m.done(DlangTypes.AT_ATTRIBUTE)
                    return false
                }
            } else {
                advance()
    ...
    fun parseCastExpression(): PsiBuilder.Marker? {
        ...
        if (!tokenCheck(DlangTypes.OP_PAR_LEFT)) {
            m.done(DlangTypes.CAST_EXPRESSION)
            return null
        }
        if (!currentIs(DlangTypes.OP_PAR_RIGHT)) {
            if (isCastQualifier) {
    ...
    fun parseDeleteExpression(): PsiBuilder.Marker? {
        ...
        if (!tokenCheck(DlangTypes.KW_DELETE)) {
            m.done(DlangTypes.DELETE_EXPRESSION)
            return null
        }
        if (parseUnaryExpression() == null) {
            m.done(DlangTypes.DELETE_EXPRESSION)
        ...
        while (!builder.eof() && builder.tokenType !== DlangTypes.OP_BRACES_RIGHT) {
            if (parseEnumMember()) {
                ...
                if (builder.tokenType === DlangTypes.OP_COMMA) {
                    ...
                    builder.advanceLexer()
                    err.error("Unexpected `,` while enum member or `}` expected")
                    continue
                }
    ...
    fun parseCommaExpression(): PsiBuilder.Marker? {
        var assign = parseAssignExpression() ?: return null
        while (currentIs(DlangTypes.OP_COMMA)) {
            assign = assign.precede()
            advance()
            if (!currentIsOneOf(DlangTypes.OP_PAR_RIGHT, DlangTypes.OP_BRACES_RIGHT, DlangTypes.OP_BRACKET_RIGHT)) {
                if (parseAssignExpression() == null) {
                    assign.done(DlangTypes.COMMA_EXPRESSION)
                    break
                }
            }
            assign.done(DlangTypes.COMMA_EXPRESSION)
    ...
    fun parseIdentityExpression(marker: PsiBuilder.Marker?): PsiBuilder.Marker? {
        ...
        if (m == null) {
            m = builder.mark()
            if (parseShiftExpression() == null) {
                m.done(DlangTypes.IDENTITY_EXPRESSION)
                return null
            }
        }
    ...
    fun parseInExpression(m: PsiBuilder.Marker?): PsiBuilder.Marker? {
        ...
        if (m == null) {
            m = builder.mark()
            if (parseShiftExpression() == null) {
                m.done(DlangTypes.IN_EXPRESSION)
                return null
            }
        }
        if (currentIs(DlangTypes.OP_NOT)) {
     ...
     * $(LITERAL 'new') $(RULE placementExpression)? $(LITERAL 'class') $(RULE arguments)? $(RULE baseClassList)? $(RULE structBody)
     * ;)
     */
    fun parseNewAnonClassExpression(marker: PsiBuilder.Marker): Boolean {
        assert(builder.tokenType === DlangTypes.KW_CLASS)
        builder.advanceLexer()
        if (currentIs(DlangTypes.OP_PAR_LEFT)) {
            builder.advanceLexer()
            if (!currentIs(DlangTypes.OP_PAR_RIGHT))
            ...
            if (!tokenCheck(DlangTypes.OP_PAR_RIGHT)) {
                marker.done(DlangTypes.NEW_ANON_CLASS_EXPRESSION)
                return false
            }
        }
        if (!currentIs(DlangTypes.OP_BRACES_LEFT)) {
     ...
     *
     * $(LITERAL 'new') $(RULE placementExpression)? $(RULE type) (($(LITERAL '[') $(RULE assignExpression) $(LITERAL ']')) | $(RULE arguments))?
     * | $(RULE newAnonClassExpression)
     * ;)
     */
    fun parseNewExpression(): PsiBuilder.Marker? {
        val m = builder.mark()
        val newAnnonClassMarker = builder.mark()
        expect(DlangTypes.KW_NEW)

        if (builder.tokenType === DlangTypes.OP_PAR_LEFT) {
            if (!parsePlacementNew()) {
                newAnnonClassMarker.drop()
                m.done(DlangTypes.NEW_EXPRESSION)
                return null
            }
        }

        if (builder.tokenType === DlangTypes.KW_CLASS) {
            val res = parseNewAnonClassExpression(newAnnonClassMarker)
            m.done(DlangTypes.NEW_EXPRESSION)
            return if (res) m else null
        }
    ...
    /**
     * Parses a placement new
     *
     * $(LITERAL '(') $(RULE assignExpression) $(LITERAL ')'
     */
    fun parsePlacementNew(): Boolean {
        assert (builder.tokenType === DlangTypes.OP_PAR_LEFT)
        val m = builder.mark()
        builder.advanceLexer()
        if (builder.tokenType === DlangTypes.OP_PAR_RIGHT) {
        ...
        expect(DlangTypes.OP_PAR_RIGHT)
        m.done(DlangTypes.PLACEMENT_NEW)
        return true
    }
    ...
    fun parseRelExpression(m: PsiBuilder.Marker?): PsiBuilder.Marker? {
        ...
        m.done(DlangTypes.REL_EXPRESSION)
        return m
    }
    ...
    fun parseTernaryExpression(): PsiBuilder.Marker? {
        ...
        if (currentIs(DlangTypes.OP_QUEST)) {
            ...
            advance()
            if (!parseExpression()) {
                m.done(DlangTypes.TERNARY_EXPRESSION)
                return null
            }
            val colon = expect(DlangTypes.OP_COLON)
            if (colon == null) {
    ...
    fun parseThrowExpression(): PsiBuilder.Marker? {
        ...
        if (parseAssignExpression() == null) {
            m.done(DlangTypes.THROW_EXPRESSION)
            return null
        }
        m.done(DlangTypes.THROW_EXPRESSION)
        return m
    ...
    fun parsePostfixExpressionTerminal(): PsiBuilder.Marker? {
    ...
    fun parsePostfixExpression0(m: PsiBuilder.Marker): PsiBuilder.Marker? {
        if (currentIs(DlangTypes.OP_DOT)) {
            val marker = m.precede()
            builder.advanceLexer()
            val bookmark = builder.mark()
    ...
    fun parsePostfixExpression(): PsiBuilder.Marker? {
        var m = parsePostfixExpressionTerminal()
        if (m == null) return m
        while (true) {
            val marker = parsePostfixExpression0(m!!) ?: break
            m = marker
        }
        return m
    }
